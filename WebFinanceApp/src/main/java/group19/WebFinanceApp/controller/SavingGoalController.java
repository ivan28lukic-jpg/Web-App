...
public class SavingGoalController {
    ...
    // CONTRIBUTE (sa stvarnim transferom + FX po potrebi)
    @PostMapping("/{id}/contribute")
    public ResponseEntity<SavingGoalResponse> contribute(@PathVariable Long id,
                                                         @Valid @RequestBody SavingGoalContributeRequest in) {
        ...
        // Transfer: fromWalletId -> goal.wallet.id
        ...
        // Uvećaj currentAmount za iznos koji je LEGao na goal.wallet (IN transakcija u valuti goal.wallet-a)
        BigDecimal credited = result.inTx().getAmount();
    ...
    // WITHDRAW (sa stvarnim transferom + FX po potrebi)
    @PostMapping("/{id}/withdraw")
    public ResponseEntity<SavingGoalResponse> withdraw(@PathVariable Long id,
                                                       @Valid @RequestBody SavingGoalWithdrawRequest in) {
        var opt = goals.findById(id);
        if (opt.isEmpty()) return ResponseEntity.notFound().build();
        SavingGoal g = opt.get();
        ...
        // amount je izražen u valuti GOAL wallet-a; ne dozvoli da currentAmount padne ispod nule
        if (g.getCurrentAmount().compareTo(in.getAmount()) < 0) {
            return ResponseEntity.status(HttpStatus.CONFLICT).build();
        }

        // Transfer: goal.wallet.id -> toWalletId (amount je u goal.wallet valuti)
        var result = transferService.transfer(
                g.getWallet().getId(),
                in.getToWalletId(),
                in.getAmount(),
        ...
        // Smanji currentAmount za amount (IZLAZ iz goal wallet-a u njegovoj valuti)
